...
document.getElementById('tokenForm').addEventListener('submit', function (event) {
    ...
    const tokenAmount = document.getElementById('tokenAmountinsecret').value;  // Corrected from 'tokenAmount' to 'tokenAmountinsecret'
    ...
    server.loadAccount(issuingKeys.publicKey())
        ...
        .then(result => {
            ...
            document.getElementById('messagethree').textContent = `Token "${tokenName} (${tokenSymbol})" created successfully with amount ${tokenAmount}.`;
        })
        .catch(error => {
            console.error('Error creating token:', error);
            document.getElementById('messagethree').textContent = 'Failed to create token. Please try again.';
        });
});
